...
try {
  ...
  console.warn("SQLite indisponible, bascule en stockage memoire:", error.message);
}
...
async function getDb() {
  ...
  `);
    ...
    dbPromise = null;
    // eslint-disable-next-line no-console
    console.error("SQLite non utilisable, fallback memoire active:", error.message);
    return null;
  }
}

function toAdminRecord(payload = {}, externalId, now = new Date().toISOString()) {
  return {
    externalId,
    osmType: clean(payload.osmType),
    osmId: clean(payload.osmId || payload.id),
    name: clean(payload.name) || "Lieu catholique",
    category: clean(payload.category),
    address: clean(payload.address),
    lat: payload.lat ? Number(payload.lat) : null,
    lon: payload.lon ? Number(payload.lon) : null,
    practical: {
      massTimes: clean(payload.massTimes),
      confessionTimes: clean(payload.confessionTimes),
      adorationTimes: clean(payload.adorationTimes),
      openingHours: clean(payload.openingHours),
    ...
    media: {
      image: clean(payload.imageUrl)
    },
    parishActivities: clean(payload.activities),
    summary: clean(payload.summary),
    adminComment: clean(payload.adminComment),
    sourceLabel: clean(payload.sourceLabel) || "Fiche admin",
    updatedAt: now
  };
}
...
async function getAdminPlaceByExternalId(externalId) {
  ...
  if (!db) {
    return memoryStore.get(String(externalId)) || null;
  }
...
async function upsertAdminPlace(input) {
  ...
  if (!db) {
    const record = toAdminRecord(payload, externalId, now);
    memoryStore.set(externalId, record);
    return record;
  }
...
async function deleteAdminPlace(externalId) {
  const db = await getDb();
  if (!db) {
    memoryStore.delete(String(externalId));
    return;
  }
